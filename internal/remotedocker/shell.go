...
// runSSH выполняет команду в новой SSH-сессии и возвращает trim'нутый stdout.
// Контекст проверяется до запуска (golang.org/x/crypto/ssh не умеет ctx-cancel
// прямо в Run, но мы хотя бы не лезем по сети при уже отменённом запросе).
...
// Помимо точного совпадения строки с маркером поддерживается «приклеенный»
// маркер: если строка ИМЕЕТ суффикс одного из маркеров (предыдущая команда
// не вывела trailing newline — частый случай у `docker inspect`), префикс
